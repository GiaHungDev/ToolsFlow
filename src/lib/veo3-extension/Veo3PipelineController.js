...
class Veo3PipelineController {
    constructor(accountData, masterService, io, browserType = 'edge', assignedProxy = null) {
        ...
        this.MAX_SLOTS = (masterService.maxVideoThreads || 0) + (masterService.maxImageThreads || 0);
        if (this.MAX_SLOTS <= 0) this.MAX_SLOTS = 3; // Default fallback
        if (this.MAX_SLOTS > 5) this.MAX_SLOTS = 5; // Hard limit

        this.workers = []; // { id, page, projectUrl, currentJob, state, failCount, startTime, percentSeen, waitingWithoutPercent, hasReloaded, viewedVideoCount }
        this.pendingQueue = [];
        this.lastSubmitTime = 0; // Để chống spam submit cùng lúc
    ...
    async clipboardPaste(locator, text) {
        try {
            if (this.browser) {
                await this.browser.grantPermissions(['clipboard-read', 'clipboard-write']).catch(() => {});
            }
    ...
    async harvestAndSaveCookies() {
        ...
        try {
            ...
            const relevantCookies = allCookies.filter(c => c.domain.includes('.google.com') || c.domain.includes('labs.google'));

            if (this.io) {
                // emit IPC to master or send to server
                this.io.emit('veo3:update-account-cookies', { accountId: this.accountData.id, cookies: relevantCookies });
            }
    ...
    async handleLoginWait() {
        ...
        try {
            ...
            if (this.accountData.loginMethod === 'tool') {
                this.log('Phương thức: Tài khoản tool. Đang kiểm tra trạng thái đăng nhập...');
                if (isLoggedIn) {
                    this.log('✅ Đăng nhập thành công qua profile Tài khoản tool!');
                    if (this.accountData && this.accountData.id && this.master && this.master.accountManager) {
                        this.master.accountManager.updateAccount(this.accountData.id, { hasProfile: true, status: 'Active' });
                    }
                    return;
                }
                
                // If not logged in, but we have credentials fetched from BAS API, let the auto-login flow run
                if (this.accountData.email && this.accountData.password) {
                    this.log('⚠️ Phát hiện session Cookies đã hết hạn. Đang tự động đăng nhập lại bằng thông tin liên kết...');
                } else {
                    // If no credentials, prompt user for manual login
                    this.log('⚠️ [HỆ THỐNG] Tài khoản tool chưa đăng nhập hoặc phiên đã hết hạn!');
                    this.log('👉 Vui lòng thực hiện đăng nhập thủ công trên cửa sổ Chrome đang mở. Bạn có 3 phút...');
                    ...
                    throw new Error('Hết thời gian chờ đăng nhập thủ công cho Tài khoản tool.');
                }
            }
            ...
            if (!isLoggedIn && !currentUrl.includes('accounts.google.com') && !currentUrl.includes('signin')) {
                ...
                if (onIntroPage) {
                    this.log('Intro page detected. Calculating exact coordinates to simulate human click...');
                    const btnCoords = await this.page.evaluate(() => {
                        ...
                        for (let el of candidates) {
                            ...
                            if (text.length > 0 && text.length < 40 && (text.includes('tạo bằng flow') || text.includes('create with flow') || text.includes('dự án mới') || text.includes('new project'))) {
                                let target = el;
                    ...
                    if (btnCoords) {
                        this.log(`Coordinates found: X=${Math.round(btnCoords.x)}, Y=${Math.round(btnCoords.y)}. Performing stealth click...`);
            ...
            if (!isLoggedIn || currentUrl.includes('accounts.google.com') || currentUrl.includes('AccountChooser') || currentUrl.includes('signin')) {
                ...
                this.log('Kiểm tra trạng thái: CHƯA ĐĂNG NHẬP. Chuẩn bị đăng nhập tự động...');
                if (this.accountData && this.accountData.id && this.master && this.master.accountManager) {
                    this.master.accountManager.updateAccount(this.accountData.id, { status: 'Login Required' });
                }
                ...
                if (this.accountData.loginType === 'auto' && email && pwd) {
                    ...
                    try {
                        ...
                        try {
                            const emailInput = this.page.locator("//input[@type='email' and @id='identifierId']");
                            ...
                            // Dùng Clipboard Paste để gõ cực kỳ nhanh và tránh hoàn toàn bộ gõ Telex của hệ điều hành
                            ...
                            const nextBtn = this.page.locator("//div[@id='identifierNext']//button");
                            ...
                            // Check error: Couldn’t find your Google Account
                            const errorEl = this.page.locator("//div[@jsname='B34EJ' and (contains(text(), 'Couldn’t find your Google Account') or contains(text(), 'Không tìm thấy tài khoản Google'))]");
                            if (await errorEl.isVisible()) {
                                this.log('Email không chính xác, hãy tắt và bật lại tool và nhập đúng email.');
                                throw new Error('Email không chính xác');
                            }
                        } catch (err) {
                            if (err.message === 'Email không chính xác') throw err;
                            this.log(`Lỗi nhập Email: ${err.message}. Có thể đã ở trang password hoặc đã đăng nhập.`);
                        }

                        // Wait for password field
                        this.log('Waiting for Password input...');
                        try {
                            const pwdInput = this.page.locator("//*[@name='Passwd'] | //input[@type='password'] | //input[@aria-label='Enter your password']").first();
                            ...
                            // Dùng Clipboard Paste để điền Password chính xác tuyệt đối
                            ...
                            const pwdNextBtn = this.page.locator("//div[@id='passwordNext']//button | //div[@id='identifierNext']//button").first();
                        ...
                        try {
                            // Give the page time to load 2FA or redirect to Labs
                            await this.sleep(5000);
                            
                            const tfaSection = this.page.locator("//section[contains(., 'Get a verification code') or contains(., 'mã xác minh') or .//input[@id='totpPin']]").first();
                            ...
                            if (await tfaSection.isVisible()) {
                                ...
                                if (tfaSecret) {
                                    ...
                                    const response = await fetch('https://tools.beeceptor.com/2FA/TOTP', {
                                        method: 'POST',
                                    ...
                                    if (response.ok) {
                                        ...
                                        if (await tfaInput.isVisible()) {
                                            ...
                                            // Dùng Clipboard Paste để điền mã 2FA nhanh nhất
                                            ...
                                            const tfaNextBtn = this.page.locator("//div[@id='totpNext']//button | //div[@id='identifierNext']//button").first();
                                            ...
                                            this.log('Không tìm thấy ô nhập 2FA (totpPin). Bỏ qua nhập tự động...');
                                        }
                                    } else {
                                        this.log('Lỗi khi gọi API 2FA: ' + response.statusText);
                                    }
                                } else {
                                    this.log('Tài khoản yêu cầu 2FA nhưng không cấu hình Secret. Bỏ qua nhập tự động...');
                                }
                            } else {
                                this.log('Không thấy màn hình yêu cầu 2FA. Có thể đã đăng nhập thành công!');
                                loginSuccess = true;
                            }
                        } catch (e) {
                        ...
                        for (let w = 0; w < 60; w++) {
                            try {
                                const isReady = await this.page.evaluate(() => {
                                    ...
                                    const btn = document.querySelector('button.sc-16c4830a-1.jsIRVP.sc-a38764c7-0.fXsrxE, button:has(i.google-symbols)');
                                    if (btn && btn.textContent && (btn.textContent.includes('New project') || btn.textContent.includes('Dự án mới'))) return true;
                                    
                                    const textNodes = Array.from(document.querySelectorAll('div, span, button'));
                                    return textNodes.some(el => el.textContent && (el.textContent.includes('Tạo bằng Flow') || el.textContent.includes('Create with Flow') || el.textContent.includes('Dự án mới') || el.textContent.includes('New Project') || el.textContent.includes('New project')));
                                });
                ...
                this.log('Kiểm tra trạng thái: ĐÃ ĐĂNG NHẬP. Bỏ qua bước điền mật khẩu.');
                if (this.accountData && this.accountData.id && this.master && this.master.accountManager) {
                    this.master.accountManager.updateAccount(this.accountData.id, { hasProfile: true, status: 'Active' });
                }
    ...
    async humanClick(page, x, y, options = {}) {
        ...
        try {
            // Nhờ có humanize: true của CloakBrowser, bạn CHỈ CẦN gọi lệnh cơ bản
            // CloakBrowser sẽ tự động tính toán đường cong Bézier siêu mượt từ vị trí hiện tại đến (x,y)
            await page.mouse.move(x, y);

            // Random pause trước khi click (người thật thường khựng lại 1 mili-giây trước khi bấm)
            await this.sleep(Math.floor(Math.random() * 80) + 40);

            // Mousedown, random delay, Mouseup
            await page.mouse.click(x, y, { button, delay: Math.floor(Math.random() * 80) + 20 });
        } catch (e) {
            this.log(`⚠️ Lỗi humanClick: ${e.message}`, 'warning');
            // TUYỆT ĐỐI KHÔNG dùng page.evaluate(() => el.click()) ở đây!
            // Google Veo3 sẽ phát hiện ra ngay lập tức vì sự kiện click qua JS sẽ có cờ isTrusted = false.
        }
    }
    ...
    async humanScroll(page) {
        ...
        try {
            // Cuộn xuống sâu
            const downScrolls = 3 + Math.floor(Math.random() * 4); // 3 đến 6 lần cuộn
            for (let i = 0; i < downScrolls; i++) {
                const distance = Math.floor(Math.random() * 1200) + 600; // 600 - 1800px mỗi lần
                await page.mouse.wheel({ deltaY: distance });
                await this.sleep(Math.floor(Math.random() * 600) + 300);
            }

            await this.sleep(Math.floor(Math.random() * 1000) + 500); // Ngừng lại xem một chút

            // Cuộn ngược lên lại
            const upScrolls = 1 + Math.floor(Math.random() * 3); // 1 đến 3 lần cuộn lên
            for (let i = 0; i < upScrolls; i++) {
                const distance = -(Math.floor(Math.random() * 1000) + 400); // -400 đến -1400px
                await page.mouse.wheel({ deltaY: distance });
    ...
    // Push new job into the queue, sort by Image priority
    addJob(job) {
        ...
        this.log(`Thêm Job ${job.id} vào hàng đợi. Tổng chờ: ${this.pendingQueue.length} jobs.`);
    }

    // Đọc danh sách Job từ file Excel và tự động đẩy vào hàng đợi
    async loadJobsFromExcel(filePath) {
        try {
            ...
            const excelFileName = path.basename(filePath, path.extname(filePath)); // VD: tap_01

            this.log(`Đã đọc được ${data.length} dòng từ file Excel: ${filePath}`);

            for (let row of data) {
                const status = (row.STATUS || row.Status || '').toString().trim().toLowerCase();
                // Bỏ qua nếu Job đã xong hoặc lỗi
                if (status === 'completed' || status === 'done' || status === 'failed') {
                    continue;
                }

                // Map các cột Excel (JOB_ID, PROMPT, IMAGE_PATH...) vào object Job
                const job = {
                    id: row.JOB_ID || row.id || row.ID || `JOB_${Date.now()}_${Math.floor(Math.random() * 1000)}`,
                    prompt: row.PROMPT || row.prompt || row.Prompt || '',
                    isImageTask: false, // Mặc định là Video
                    ...
                    projectId: excelFileName, // Group tất cả các prompt trong file này vào chung 1 Project

                    // Metadata phục vụ lưu file
                    excelFilePath: filePath,
                    excelFileName: excelFileName,
                    targetFileName: `video_${row.JOB_ID || row.id || row.ID}.mp4`
                };

                // Tránh lỗi nếu không có JOB_ID trong row
    ...
    async initBrowser() {
        ...
        if (this.accountData.loginMethod === 'tool' && this.accountData.toolAccount) {
            const safeToolName = this.accountData.toolAccount.replace(/[^a-zA-Z0-9_]/g, '_');
            if (profileBasePath) {
                profileBasePath = path.join(profileBasePath, `profile_tool_${safeToolName}`);
            } else {
            ...
            const baseDir = process.env.USER_DATA_PATH || 'C:\\Profiles_BAS_Flow';
            profileBasePath = path.join(baseDir, `profile_account_${this.accountData.id}`);
        }
        ...
        if (this.accountData && this.accountData.headless !== undefined && this.accountData.headless !== null) {
            isHeadless = String(this.accountData.headless) !== 'false' && this.accountData.headless !== 0 && this.accountData.headless !== false;
        }
        ...
        if (useRealChrome) {
            this.log(`✅ Phát hiện Chrome thật tại: ${chromePath}. Đang sử dụng Chrome chính chủ`);
        } else {
            this.log(`⚠️ Không tìm thấy Chrome thật tại ${chromePath}. Đang dùng Chromium mặc định của CloakBrowser...`);
        }
        ...
        this.browser = await launchPersistentContext({
            ...
            args: browserArgs,
            viewport: null, // Playwright way to maximize
            proxy: proxyOpt,
            humanize: true, // Bật tính năng giả lập người thật của CloakBrowser
            launchOptions: {
                executablePath: useRealChrome ? chromePath : undefined,
        ...
        // NẠP COOKIES VÀO TRÌNH DUYỆT TỪ BIẾN ACCOUNT
        let cookieArr = null;
        if (this.accountData && this.accountData.cookies) {
            if (Array.isArray(this.accountData.cookies)) {
                cookieArr = this.accountData.cookies;
            } else if (this.accountData.cookies.cookies && Array.isArray(this.accountData.cookies.cookies)) {
                cookieArr = this.accountData.cookies.cookies; // Hỗ trợ format Export từ Extension J2Team / EditThisCookie
            }
        }

        // XÓA COOKIES ĐỂ TRÁNH TRÙNG LẶP SESSION CŨ (Chỉ áp dụng khi dùng Account hoặc Cookies mới, KHÔNG áp dụng cho Tài khoản tool)
        if (this.accountData.loginMethod !== 'tool') {
            this.log('Đang xóa toàn bộ cookies cũ trong trình duyệt để khởi tạo session mới...');
            try {
            ...
            this.log('Phương thức Tài khoản tool: Giữ nguyên session cookies đã lưu trong profile.');
        }

        if (cookieArr && cookieArr.length > 0) {
            this.log(`Tìm thấy ${cookieArr.length} cookies, đang thiết lập vào trình duyệt...`);
            try {
                // Xóa các trường không được Playwright hỗ trợ và chuẩn hóa domain
                const cleanCookies = cookieArr.map(c => {
                    const { hostOnly, session, storeId, ...rest } = c;
                    if (rest.sameSite === 'unspecified') delete rest.sameSite;
                    if (rest.sameSite && !['Strict', 'Lax', 'None'].includes(rest.sameSite)) delete rest.sameSite;
    ...
    async runLoop() {
        ...
        while (this.isRunning) {
            try {
                // 1. Khởi tạo đủ số lượng Worker (Tab)
                while (this.workers.length < this.MAX_SLOTS && this.isRunning) {
                    const isFirst = this.workers.length === 0;
                    // Tab đầu tiên tận dụng luôn this.page đã login
                    const newPage = isFirst ? this.page : await this.browser.newPage();
                    if (!isFirst) {
                        await newPage.bringToFront();
                        // Chờ một chút để Google không chú ý việc mở tab liên tục
                        await this.sleep(2000);
                    }
                    this.workers.push({
                        id: this.workers.length + 1,
                        page: newPage,
                        projectUrl: null, // Sẽ được set khi tạo project xong
                        currentJob: null,
                        state: 'idle', // 'idle', 'submitting', 'polling'
                        failCount: 0,
                        startTime: 0,
                        percentSeen: false,
                ...
                // 2. Phân bổ Job mới vào các Worker đang rảnh (idle)
                for (let worker of this.workers) {
                    if (worker.state === 'idle' && this.pendingQueue.length > 0) {
                        ...
                        // Chạy submit không block luồng chính
                        this.processWorkerSubmit(worker).catch(async e => {
                            this.log(`[Worker ${worker.id}] Lỗi submit Job ${nextJob.id}: ${e.message}`, 'error');
                            
                            nextJob.retries = (nextJob.retries || 0) + 1;
                            if (nextJob.retries === 1) {
                                this.log(`[Worker ${worker.id}] 🔄 Job ${nextJob.id} bị lỗi lần đầu. Tự động đẩy lại vào đầu hàng đợi để thử lại ngay lập tức...`);
                                this.pendingQueue.unshift(nextJob);
                            } else {
                                this.log(`[Worker ${worker.id}] ❌ Job ${nextJob.id} đã thất bại lần thứ 2. Cập nhật trạng thái Thất bại lên server.`);
                                if (this.master) {
                            ...
                            if (worker.failCount >= 3) {
                                this.log(`[Worker ${worker.id}] Lỗi liên tiếp 3 lần. Đóng tab này và GIẢM 1 luồng xử lý trong 3 phút...`, 'error');
                                await worker.page.close().catch(() => { });
                                this.workers = this.workers.filter(w => w.id !== worker.id);
                                this.MAX_SLOTS--; // Tự động giảm luồng tạm thời
                                
                                setTimeout(() => {
                                    this.MAX_SLOTS++;
                                    this.log(`[HỆ THỐNG] 🕒 Đã hết 3 phút giảm luồng. Tự động khôi phục lại luồng xử lý. Số luồng tối đa hiện tại: ${this.MAX_SLOTS}`);
                                }, 180000);
                            } else {
                                worker.state = 'idle'; // Trở lại trạng thái rảnh để nhận Job mới
                ...
                // 3. Tuần tra kết quả (Polling Phase) cho các Worker đang chờ render
                for (let worker of this.workers) {
                    if (worker.state === 'polling') {
                        if (Date.now() - worker.startTime > 10 * 60 * 1000) {
                            this.log(`[Worker ${worker.id}] Timeout: Job ${worker.currentJob.id} bị treo hơn 10 phút.`);
                            
                            worker.currentJob.retries = (worker.currentJob.retries || 0) + 1;
                            if (worker.currentJob.retries === 1) {
                                this.log(`[Worker ${worker.id}] 🔄 Job ${worker.currentJob.id} bị timeout lần đầu. Tự động đẩy lại vào đầu hàng đợi để thử lại ngay lập tức...`);
                                this.pendingQueue.unshift(worker.currentJob);
                            } else {
                                this.log(`[Worker ${worker.id}] ❌ Job ${worker.currentJob.id} đã bị timeout lần thứ 2. Cập nhật trạng thái Thất bại lên server.`);
                                this.master.updateJobStatus(worker.currentJob, 'Failed', "Timeout 10 phút không có kết quả");
                            }
                        ...
                        try {
                            ...
                            if (isDone) {
                                this.log(`[Worker ${worker.id}] Job ${worker.currentJob.id} đã hoàn tất. Worker trở lại trạng thái rảnh.`);
                                this.master.activeJobs.delete(worker.currentJob.id);
                                worker.state = 'idle';
                                worker.currentJob = null;
                            }
                        } catch (e) {
                            this.log(`[Worker ${worker.id}] Lỗi khi kiểm tra trạng thái Job: ${e.message}`, 'error');
                            
                            worker.currentJob.retries = (worker.currentJob.retries || 0) + 1;
                            if (worker.currentJob.retries === 1) {
                                this.log(`[Worker ${worker.id}] 🔄 Job ${worker.currentJob.id} bị lỗi kiểm tra lần đầu. Tự động đẩy lại vào đầu hàng đợi để thử lại ngay lập tức...`);
                                this.pendingQueue.unshift(worker.currentJob);
                            } else {
                                this.log(`[Worker ${worker.id}] ❌ Job ${worker.currentJob.id} lỗi kiểm tra lần thứ 2. Cập nhật trạng thái Thất bại lên server.`);
                                this.master.updateJobStatus(worker.currentJob, 'Failed', e.message);
                            }
                            this.master.activeJobs.delete(worker.currentJob.id);
            ...
            await this.sleep(3000); // Nghỉ 3 giây trước chu kỳ tuần tra tiếp theo
        }
    }
    ...
    async clickModelDropdownWithVerify(page, clickCoord, coords, triggerKey, modelName) {
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            ...
            if (newMenuOpened) {
                ...
                // Click vào input thay vì click random ra ngoài để đóng menu bị kẹt
                const editorCoords = await this.findNodeBySelector(page, '[data-slate-editor="true"]');
                if (editorCoords) {
    ...
    async selectImagesFromGallery(page, rawImagePaths, job = null) {
                ...
                if (p.startsWith('{')) {
                    try {
                        ...
                        if (parsed.veo3Id) {
                            ...
                            throw new Error("Thiếu veo3Id trong dữ liệu ảnh. Vui lòng tạo lại nhân vật bằng công cụ AI để đồng bộ ID.");
                        }
                    } catch (e) {
                        if (e.message.includes('veo3Id')) throw e;
                    }
                } else if (p.trim()) {
                    throw new Error("Đường dẫn ảnh không hợp lệ (Không phải dữ liệu JSON chứa veo3Id).");
                }
            }
        }
        ...
        this.log(`Bắt đầu chọn ${veo3Ids.length} ảnh nhân vật từ Thư viện (Gallery) cho mode IN2V/I2V...`);
        ...
        for (let wait = 0; wait < 15; wait++) {
            plusBtnCoords = await page.evaluate(() => {
                ...
                // 1. Tìm thẻ cha chung (Container) chứa Editor, mở rộng dần lên trên (tối đa 5 cấp)
                let parent = editor.parentElement;
                let validBtns = [];
                for (let i = 0; i < 5; i++) {
                    if (!parent) break;
                    // Tìm tất cả các button trong parent này
                    const btns = Array.from(parent.querySelectorAll('button, [role="button"], div[role="button"]'));
                    validBtns = btns.filter(b => {
                        // Phải hiển thị được
                        const r = b.getBoundingClientRect();
                        if (r.width === 0 || r.height === 0) return false;

                        // Loại bỏ chính editor
                        if (b === editor || b.contains(editor)) return false;

                        return true;
                    });

                    // Nếu parent đã gom được nhiều hơn 0 button, ta coi đây là Input Container
                ...
                // 2. Trong các nút cùng Container, tìm nút + (Nút đính kèm)
                const editorRect = editor.getBoundingClientRect();
                let bestBtn = null;
                let minDistance = 9999;

                for (let b of validBtns) {
                    // Tiêu chí tìm kiếm chính xác theo DOM: aria-haspopup="dialog" và chứa icon "add_2" hoặc "add"
                    const hasDialogAria = b.getAttribute('aria-haspopup') === 'dialog';

                    const hasGooglePlusIcon = Array.from(b.querySelectorAll('i, span, div')).some(el => {
                        if (el.classList && Array.from(el.classList).some(c => c.includes('google-symbols'))) {
                    ...
                    // Kiểm tra thẻ span ẩn chứa chữ Tạo hoặc Create
                    const hiddenSpan = b.querySelector('span[style*="absolute"]');
                    const isCreateText = hiddenSpan && ['tạo', 'create'].includes((hiddenSpan.textContent || '').trim().toLowerCase());

                    if ((hasDialogAria && hasGooglePlusIcon) || (hasGooglePlusIcon && isCreateText)) {
                        bestBtn = b;
                        break;
                    }

                    // Fallback: Kiểm tra aria-label upload
                    const aria = (b.getAttribute('aria-label') || '').toLowerCase();
                    if (aria.includes('tải lên') || aria.includes('upload') || aria.includes('đính kèm')) {
                        bestBtn = b;
                        break;
                    }
                }
                ...
                // 3. Fallback nếu không có thuật toán khoảng cách: Tìm thẳng bằng aria
                const explicitUploadBtn = validBtns.find(b => {
                    const aria = (b.getAttribute('aria-label') || '').toLowerCase();
                    return aria.includes('tải lên') || aria.includes('upload') || aria.includes('attach');
                });
        ...
        if (!plusBtnCoords) {
            throw new Error('TIMEOUT: Không tìm thấy nút (+) để đính kèm ảnh/giọng nói sau 15 giây. Dừng Job!');
        }
        ...
        if (plusBtnCoords) {
            ...
            if (veo3Ids.length > 0) {
                this.log(`Tiến hành chọn ${veo3Ids.length} nhân vật từ Gallery bằng ID...`);
                ...
                const changeGallerySort = async (mode) => {
                    return await page.evaluate(async (m) => {
                        ...
                        while ((node = tw.nextNode())) {
                            const t = node.nodeValue.trim().toLowerCase();
                            if (t === 'gần đây' || t === 'mới nhất' || t === 'cũ nhất' || t === 'dùng nhiều nhất') {
                                // Tìm thẻ cha có thể click được
                                targetSortBtn = node.parentElement.closest('button, div[role="button"], div[role="combobox"], div[aria-haspopup], span');
                        ...
                        if ((targetSortBtn.innerText || targetSortBtn.textContent || '').trim().toLowerCase() === m.toLowerCase()) {
                            return true;
                        }

                        // Mở menu
                        targetSortBtn.click();
                        await sleep(800); // Chờ menu xổ ra (đôi khi có hiệu ứng animation)

                        // Tìm option mong muốn trong menu bằng Text Node
                        const optTw = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
                        let optNode;
                        while ((optNode = optTw.nextNode())) {
                            if (optNode.nodeValue.trim().toLowerCase() === m.toLowerCase()) {
                                ...
                                const optEl = optNode.parentElement.closest('div[role="option"], li[role="option"], li, span');
                                if (optEl && optEl !== targetSortBtn && optEl.clientHeight > 0) {
                                    optEl.click();
                                    await sleep(2000); // Chờ gallery load lại
                                    return true;
                                }
                            }
                        }

                        // Không thấy option, click ra ngoài đóng menu
                        document.body.click();
                        return false;
                    }, mode);
                };
                ...
                const searchWithScroll = async (vid) => {
                    return await page.evaluate(async (id) => {
                        ...
                        const findAndClick = () => {
                            const imgs = Array.from(document.querySelectorAll('div[role="dialog"] img, div[data-radix-popper-content-wrapper] img'));
                        ...
                        while (attempts < maxAttempts) {
                            ...
                            const dialog = document.querySelector('div[role="dialog"]') || document.querySelector('div[data-radix-popper-content-wrapper]') || document.body;
                ...
                for (const vid of veo3Ids) {
                    this.log(`Đang tìm kiếm ảnh ID: ${vid} trong Gallery (có thể tự động cuộn)...`);

                    let clicked = await searchWithScroll(vid);

                    if (!clicked) {
                        this.log(`Không thấy ảnh ID: ${vid}. Đang thử đổi sắp xếp sang "Mới nhất"...`);
                        await changeGallerySort('mới nhất');
                        clicked = await searchWithScroll(vid);
                    }
                    if (!clicked) {
                        this.log(`Không thấy ảnh ID: ${vid}. Đang thử đổi sắp xếp sang "Cũ nhất"...`);
                    ...
                    if (clicked) {
                        ...
                        throw new Error(`Không tìm thấy ảnh nhân vật (ID: ${vid}) trong Thư viện (Gallery) của Google Veo3. Vui lòng tạo lại ảnh.`);
                    }
                }
            } else {
                this.log('Không có ảnh nguyên liệu cần chọn từ thư viện, bỏ qua bước chọn ảnh.');
                selectSuccess = true;
            }
        } else {
        ...
        if (selectSuccess && veo3Ids.length > 0) {
            this.log(`Đã chọn ảnh thành công. Bắt đầu chờ đính kèm hoàn tất (Tối đa 15s)...`);
            ...
            if (!looksDone) {
                this.log('⚠️ Hết timeout chờ ảnh đính kèm. Thumbnail không xuất hiện. Tiếp tục luồng xử lý...');
            } else {
                this.log('Đang chờ thêm 5 giây để Google load hoàn tất file ảnh trước khi cho phép bấm Submit...');
    ...
    async uploadLocalImagesIN2V(page, imagesArray) {
        ...
        for (let imgObj of imagesArray) {
            for (let key in imgObj) {
                const cleanPath = imgObj[key] ? imgObj[key].replace(/[\u200B-\u200D\uFEFF\u202A-\u202E]/g, '').replace(/^["']|["']$/g, '').trim() : null;
        ...
        for (let i = 0; i < paths.length; i++) {
            const path = paths[i];
            this.log(`Đang tìm nút Add Image (add_2) để upload ảnh ${i + 1}/${paths.length}...`);

            // Step 1: Find and click add_2
            ...
            try {
                const [fileChooser] = await Promise.all([
                    ...
                    page.evaluate(() => {
                        ...
                        for (let div of divs) {
                            ...
                            if (t === 'upload image' || t === 'tải hình ảnh lên' || t === 'tải lên' || t === 'tải hình ảnh') {
                ...
                if (fileChooser) {
                    ...
                    while (!uploadDone && waitChecks < 40) { // Max 60s
                        const state = await page.evaluate(() => {
                            ...
                            const hasPercent = allNodes.some(el => {
                                ...
                                return t.endsWith('%') && t.length > 1 && t.length <= 4 && !isNaN(parseInt(t.replace('%', '')));
                            });
                            return { hasPercent };
                        });
                        ...
                        if (!uploadDone) {
                            ...
                            await page.mouse.move(cx + r * Math.cos(angle), cy + r * Math.sin(angle)).catch(() => { });
                            await this.sleep(1500);
                        }
                    }

                    if (uploadDone) {
                        this.log(`✅ Ảnh ${i + 1} đã upload hoàn tất (dựa trên trạng thái hiển thị %).`);
                    } else {
                        this.log(`⚠️ Hết thời gian chờ xử lý ảnh ${i + 1}. Vẫn tiếp tục thực hiện.`);
                    }
                }
            } catch (err) {
                this.log(`⚠️ Lỗi: Không mở được File Chooser cho ảnh thứ ${i + 1}. Error: ${err.message}`);
            }
        }
        return true;
    }
        ...
        try {
            ...
            if (targetUrl) {
                ...
                if (!currentUrl.includes(targetUrl)) {
                    this.log(`[Worker ${worker.id} - BƯỚC 0/5] Chuyển hướng về Project Flow hiện có: ${targetUrl}`);
                    await page.goto(targetUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
                    await this.sleep(3000 + Math.floor(Math.random() * 2000));

                    if (page.url().includes('accounts.google.com/v3/signin')) {
                        throw new Error("Cookies hết hạn vui lòng nhập cookies mới để tiếp tục");
                    }
                } else {
                    this.log(`[Worker ${worker.id} - BƯỚC 0/5] Đang ở sẵn giao diện Project Flow, tiếp tục nạp lệnh...`);
                }
            } else {
                // Chưa có Project URL -> Tạo mới
                this.log(`[Worker ${worker.id} - BƯỚC 0/5] Bắt đầu Job bằng cách tạo Project mới...`);
                ...
                // Tìm nút Dự án mới
                this.log(`[Worker ${worker.id}] Tìm nút "Dự án mới" hoặc "Tạo bằng Flow" để bắt đầu Job...`);
                const newProjCoords = await page.evaluate(() => {
                    const candidates = Array.from(document.querySelectorAll('a, button, div[role="button"], span, div'));
                    for (let el of candidates) {
                        const text = (el.innerText || el.textContent || '').trim().toLowerCase();
                        if (text.length > 0 && text.length < 40 && (text.includes('dự án mới') || text.includes('new project') || text.includes('tạo bằng flow') || text.includes('create with flow'))) {
                            let target = el;
                            while (target && target.tagName !== 'A' && target.tagName !== 'BUTTON' && target.getAttribute('role') !== 'button') {
                                if (!target.parentElement || target.tagName === 'BODY') break;
                                target = target.parentElement;
                ...
                if (newProjCoords) {
                    this.log(`[Worker ${worker.id}] Tìm thấy nút bắt đầu Project. Tiến hành click...`);
                    await this.humanClick(page, newProjCoords.x, newProjCoords.y);
                }

                // Wait for redirect to project url
                this.log(`[Worker ${worker.id}] Đang đợi Google tạo Project và redirect...`);
                currentUrl = page.url();
                let retries = 0;
                ...
                if (!currentUrl.includes('/project/')) {
                    ...
                    throw new Error("Timeout khi đợi tạo Project mới. URL hiện tại: " + currentUrl);
                }

                targetUrl = currentUrl;
                this.log(`[Worker ${worker.id}] Đã tạo thành công Project mới: ${targetUrl}`);
            }

            // 2. Wait for Editor to load
            await page.waitForSelector('[data-slate-editor="true"]', { timeout: 15000 });
            ...
            const coords = {
                ...
                model: {
                    ...
                    trigger_image: { type: 'text', value: ['🍌 Nano Banana Pro', 'Nano Banana Pro', '🍌 Nano Banana 2', 'nano banana 2', 'Imagen 3', 'Imagen 3 Fast', 'Imagen'] },
            ...
            if (coords.modes[TYPE_VIDEO]) {
                ...
                if (!clickedMode) {
                    throw new Error(`Không tìm thấy chế độ ${TYPE_VIDEO} trong Config Menu. Hủy Job!`);
                }
                await this.sleep(1000 + Math.floor(Math.random() * 800));

                if (TYPE_VIDEO === 'IN2V' || TYPE_VIDEO === 'I2V') {
                    this.log(`[BƯỚC 2b/5] Đang chuyển sang tab thành phần / khung hình phụ trợ...`);
                    const clickedSubMode = await clickDynamicNode(coords.subModes, TYPE_VIDEO);

                    if (!clickedSubMode) {
                        throw new Error(`Không tìm thấy Tab phụ trợ cho ${TYPE_VIDEO} trong Config Menu. Có thể Google Veo đã thay đổi giao diện. Hủy Job!`);
                    }
                ...
                if (currentSettings) {
                    ...
                    if (isImg) {
                        ...
                        if (currentSettings.model) await this.clickModelDropdownWithVerify(page, clickCoord, coords, 'trigger_image', currentSettings.model);
                    } else {
                        ...
                        if (currentSettings.model) await this.clickModelDropdownWithVerify(page, clickCoord, coords, 'trigger_video', currentSettings.model);
                    }
                }
            }
            ...
            const processImagePath = (imgPath) => {
                ...
                if (imgPath.startsWith('data:image/')) {
                    ...
                    const tmpPath = path.join(tmpDir, `veo3_in2v_${crypto.randomBytes(8).toString('hex')}.${ext}`);
            ...
            if (TYPE_VIDEO === 'IN2V') {
                if (job.images && job.images.length > 0) {
                    this.log(`[BƯỚC 4/5] Phát hiện Local Images cho IN2V. Tiến hành upload trực tiếp...`);
                    selectResult = await this.uploadLocalImagesIN2V(page, job.images);
                } else {
                    selectResult = await this.selectImagesFromGallery(page, [IMAGE_PATH, IMAGE_PATH_2, IMAGE_PATH_3], job);
                }
            ...
            if (TYPE_VIDEO === 'IN2V' && selectResult === false) {
                throw new Error("Không thể chọn ảnh nhân vật từ Gallery. Hủy Job để tránh sinh sai kết quả.");
            }

            // Add Job ID signature to prompt
            const trackingSignature = `\n\n[Ignore this: JobID=${job.id}]`;
            let cleanPrompt = prompt.replace(/--ar\s+\d+[:-]\d+/gi, '').replace(/--ar \d+\/\d+/gi, '').trim() + trackingSignature;

            this.log(`[Worker ${worker.id} - BƯỚC 5/5] Chuẩn bị Paste câu lệnh (Prompt) và Submit... (Length: ${cleanPrompt.length})`);
            ...
            if (timeSinceLastSubmit < requiredStaggerTime) {
                ...
                this.log(`[Worker ${worker.id}] Đang căn chỉnh gửi xen kẽ. Chờ thêm ${Math.floor(waitTime / 1000)}s rồi mới nhập lệnh...`);
            ...
            this.log(`[Worker ${worker.id} - BƯỚC 5b/5] Giả lập hành vi người dùng: Cuộn trang và click an toàn trước khi bấm Submit...`);
            ...
            await page.mouse.click(Math.floor(Math.random() * 150) + 10, Math.floor(Math.random() * 50) + 10);
            await this.sleep(500 + Math.floor(Math.random() * 1000));

            // Click Generate
            this.log(`[SUBMIT] Bấm Submit (arrow_forward)...`);
            ...
            this.log('[CHỜ KẾT QUẢ] Đang chờ hệ thống xác nhận đã gửi lệnh (Đang tạo / % Render)...');
            let isSubmitted = false;
            let errorToast = null;

            for (let check = 0; check < 15; check++) { // Chờ tối đa 15 giây
                await this.sleep(1000);

                const status = await page.evaluate(() => {
                    // 1. Kiểm tra báo lỗi (Policy, Network, v.v.)
                    ...
                    // 2. Kiểm tra tiến trình đang chạy (Đang tạo / Generating / 0-100%)
                    const texts = Array.from(document.querySelectorAll('span, div, p'));
                    const isRunning = texts.some(el => {
                        if (el.offsetParent === null) return false;
                        const t = el.textContent.trim();
                        if (t.includes('Đang tạo') || t.includes('Generating') || t.includes('Creating') || t.includes('Queued')) return true;
                        return t.endsWith('%') && t.length > 1 && t.length <= 5 && !isNaN(parseInt(t));
                    });

                    if (isRunning) return { type: 'running' };

                    // 3. Fallback: Xem ô text editor có bị clear không
            ...
            if (errorToast) {
                const errDir = this.accountData.outputDir || path.join(this.profilePath, '..', 'outputs');
                if (!fs.existsSync(errDir)) fs.mkdirSync(errDir, { recursive: true });
                const errPic = path.join(errDir, `error_policy_${job.id}_${Date.now()}.png`);
                await page.screenshot({ path: errPic, fullPage: true }).catch(() => { });
                throw new Error(`Google từ chối Prompt: "${errorToast}". Màn hình: ${errPic}`);
            }

            if (!isSubmitted) {
                const errDir = this.accountData.outputDir || path.join(this.profilePath, '..', 'outputs');
                if (!fs.existsSync(errDir)) fs.mkdirSync(errDir, { recursive: true });
                const errPic = path.join(errDir, `error_submit_${job.id}_${Date.now()}.png`);
                await page.screenshot({ path: errPic, fullPage: true }).catch(() => { });

                throw new Error(`Job không thể Submit (Nút Generate không hoạt động hoặc Tool bị kẹt). Xem ảnh: ${errPic}`);
            }

            await this.sleep(3000); // Chờ Google submit thành công lên Server
    ...
    async checkJobStatus(worker) {
        ...
        this.log(`[Worker ${worker.id}] Đang kiểm tra tiến độ Job ${job.id} tại: ${projectUrl}`);

        try {
            // Điều hướng tới URL của Project nếu chưa ở đó
            if (!page.url().includes(projectUrl)) {
                await page.goto(projectUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
                await this.sleep(4000); // Chờ UI Render hoàn toàn
            }

            const evaluateResult = await page.evaluate((jobId) => {
                // 1. Kiểm tra Lỗi Unusual Activity trước tiên
                const allDivs = document.querySelectorAll("div, span, p");
                for (let i = 0; i < allDivs.length; i++) {
                    const text = (allDivs[i].textContent || allDivs[i].innerText || "").trim().toLowerCase();
                    if (text.includes("we noticed some unusual activity")) {
                        return { status: 'unusual_activity' };
                    }
                }

                // 2. Kiểm tra Lỗi chung trên toàn trang (có chữ Lỗi/Vi phạm)
                const alerts = Array.from(document.querySelectorAll('[role="alert"], snack-bar, .msg, .error, .toast'));
                for (let a of alerts) {
                    if (a.offsetParent !== null) {
                        const t = (a.innerText || "").toLowerCase();
                        if (t.includes('vi phạm') || t.includes('chính sách') || t.includes('lỗi') || t.includes('error') || t.includes('không thể')) {
                            return { status: 'error', msg: a.innerText.trim() };
                        }
                    }
                }

                // 2. Tìm kiếm ký tự % trên TOÀN TRANG (giống y hệt đoạn code BAS của bạn)
                let hasPercent = false;
                let currentPercent = '';
                const percentDivs = document.querySelectorAll("div, span, p");
                for (let i = 0; i < percentDivs.length; i++) {
                    const text = (percentDivs[i].textContent || percentDivs[i].innerText || "").trim();
                    if (text.indexOf("%") >= 0 && text.length <= 5) {
                        hasPercent = true;
                        currentPercent = text;
                        break;
                    }
                    if (text === 'Đang tạo' || text === 'Generating' || text === 'Creating' || text === 'Queued') {
                        hasPercent = true;
                        break;
                    }
                }
            ...
            if (evaluateResult.status === 'unusual_activity') {
                this.log(`⚠️ Phát hiện lỗi "Unusual activity". Đang chờ 2s để bấm nút Go Back...`);
                await this.sleep(2000);

                // Click nút Go Back lần 1
                ...
                // Kiểm tra xem có còn lỗi Unusual activity không
                ...
                if (stillError) {
                    this.log('⚠️ Vẫn còn lỗi Unusual activity. Đang click Go Back lần 2 để ra ngoài...');
                    ...
                    this.log(`[Worker ${worker.id}] Đã F5. Chờ ngẫu nhiên ${Math.floor(waitTime / 1000)}s trước khi tạo project mới...`);
                    await this.sleep(waitTime);

                    this.log(`[Worker ${worker.id}] Tiến hành tạo Project mới và submit lại Job từ đầu...`);
                    worker.projectUrl = null; // Xóa cache URL cũ để ép tạo mới
                    const newProjectUrl = await this.submitNewJob(worker);
                    if (newProjectUrl) {
                        worker.projectUrl = newProjectUrl;
                        worker.startTime = Date.now();
                    } else {
                        throw new Error('Không thể tạo Project mới sau khi phục hồi từ Unusual Activity.');
                    }
                    return false;
                } else {
                    this.log(`[Worker ${worker.id}] Đã hết lỗi Unusual Activity, đang ở trong Project. Đợi 5s và xoay chuột...`);

                    let editorCoords = null;
                    for (let i = 0; i < 10; i++) {
                        editorCoords = await this.findNodeBySelector(page, '[data-slate-editor="true"]');
                    ...
                    let submitCoords = await this.findNodeByTextExact(page, ['arrow_forward']);
                    if (submitCoords) {
                    ...
                    worker.percentSeen = false;
                    worker.waitingWithoutPercent = 0;
                    return false; // Vẫn đang xử lý
                }
            }

            if (evaluateResult.status === 'generating') {
                worker.percentSeen = true; // Đánh dấu là đã bắt đầu chạy % (tránh F5 quá sớm lúc vừa submit)
                worker.waitingWithoutPercent = 0; // Reset bộ đếm
                if (evaluateResult.percent) {
                    this.log(`[Worker ${worker.id}] Tiến trình Job ${job.id}: ${evaluateResult.percent}`);
                }
                this.master.updateJobStatus(job, 'Rendering');

                // --- HUMANIZING: Randomly click an existing video to view ---
                worker.viewedVideoCount = worker.viewedVideoCount || 0;
                // Xác suất 15% click xem video, tối đa 2 lần mỗi job
                if (worker.viewedVideoCount < 2 && Math.random() < 0.15) {
                    const videoCoords = await page.evaluate(() => {
                        ...
                        if (validVideos.length > 0) {
                            const randVid = validVideos[Math.floor(Math.random() * validVideos.length)];
                    ...
                    if (videoCoords) {
                        ...
                        const backCoords = await page.evaluate(() => {
                            ...
                            for (let b of btns) {
                                const txt = (b.innerText || '').toLowerCase();
                                if (txt.includes('quay lại') || txt.includes('go back') || b.innerHTML.includes('arrow_back')) {
                                    const r = b.getBoundingClientRect();
                                    if (r.width > 0 && r.height > 0) return { x: r.x + r.width / 2, y: r.y + r.height / 2 };
                                }
                            }
                ...
                return false; // Vẫn đang render
            }

            if (evaluateResult.status === 'done_rendering') {
                // Nếu chưa từng thấy %, chứng tỏ Job vừa mới submit xong chưa kịp hiện %.
                // Ta phải chờ thêm vài giây, nếu quá 10 lần check (30 giây) mà vẫn ko có % thì mới coi là xong thật.
                if (!worker.percentSeen) {
                    worker.waitingWithoutPercent = (worker.waitingWithoutPercent || 0) + 1;
                    if (worker.waitingWithoutPercent < 10) {
                        this.log(`[Worker ${worker.id}] Job ${job.id} đang chờ hệ thống xếp hàng (chưa thấy tiến trình)...`);
                        return false;
                    }
                } else {
                    // Nếu đã thấy % rồi mà đột nhiên biến mất, chờ 5s để xác nhận chắc chắn không phải do UI lag
                    await this.sleep(5000);
                }
            }

            // 5. NẾU ĐẾN ĐÂY: Đã mất chữ % -> Render xong!
            // Thực hiện F5 lại trang trước khi gọi API theo yêu cầu
            if (!worker.hasReloaded) {
                this.log(`[Worker ${worker.id}] Job ${job.id} đã hoàn thành render. Đang F5 lại trang để đồng bộ API...`);
                await page.reload({ waitUntil: 'domcontentloaded', timeout: 30000 });
                await this.sleep(8000); // Tăng thời gian chờ lên 8s để quan sát rõ hơn
                worker.hasReloaded = true; // Đánh dấu đã F5 để không F5 liên tục
            }
            ...
            const mediaUrl = await page.evaluate(async ({ jobId, projectId }) => {
                try {
                        ...
                        for (let j = 0; j < steps.length; j++) {
                            ...
                            if (prompt.includes(`JobID=${jobId}`)) {
                                ...
                                for (let k = 0; k < gens.length; k++) {
                                    let url = gens[k].mediaData?.videoData?.fifeUri || gens[k].mediaData?.imageData?.fifeUri || "";
                                    if (url) return url;
                                }
                            }
                        }
            ...
            if (!mediaUrl) {
                slot.apiRetries = (slot.apiRetries || 0) + 1;
                if (slot.apiRetries > 3) {
                    throw new Error("Không thể trích xuất Media URL từ API sau 3 lần thử. Khả năng cao Google đã chặn hoặc render thất bại ngầm. Bỏ qua Job này.");
                }
                this.log(`Chưa lấy được link từ API (Thử lần ${slot.apiRetries}/3). Sẽ thử lại ở vòng lặp sau...`);
                return false;
            }

            // ĐÃ CÓ URL MEDIA -> TIẾN HÀNH DOWNLOAD
            this.log(`Job ${job.id} ĐÃ HOÀN THÀNH (100%)! Tìm thấy link tải từ API: ${mediaUrl.substring(0, 50)}...`);
            await this.sleep(3000);
            this.master.updateJobStatus(job, 'Downloading');

            // Xác định đường dẫn lưu file
            const isCharacterJob = job.typeI2V === 'Character' || job.typeI2V === 'Avatar' || !!job.characterId;
            let finalDir;
            if (job.excelFilePath) {
                // Lưu vào thư mục chứa file Excel -> tạo thư mục con trùng tên file Excel
                const excelDir = path.dirname(job.excelFilePath);
                finalDir = path.join(excelDir, job.excelFileName || job.id.toString());
            } else {
                // Fallback nếu không chạy từ Excel
                const outputDir = typeof this.master.getJobOutputDir === 'function'
                    ? this.master.getJobOutputDir(job.projectId, isCharacterJob, job.outputDir)
                    : (job.outputDir ? path.join(this.master.outputDir, job.outputDir) : this.master.outputDir);
                finalDir = outputDir;
            ...
            // Override finalDir to save in C:\{projectName} as requested
            const safeProjectName = (job.projectName || job.projectId || 'Veo3_Downloads').replace(/[<>:"/\\|?*]+/g, '_');
            ...
            // Bỏ qua bước quét thẻ HTML cũ, sử dụng trực tiếp URL lấy từ API
            ...
            if (!base64Data || base64Data.error) {
                ...
                const errorMsg = base64Data && base64Data.error ? base64Data.error : 'Unknown';
                this.log(`⚠️ Không lấy được thông tin media cho Job ${job.id}! Lỗi: ${errorMsg}. Thử lại sau.`);
                if (errorMsg !== "WAIT_FOR_IMAGE_LOAD") {
                    job.downloadRetries = (job.downloadRetries || 0) + 1;
                    if (job.downloadRetries >= 5) {
                        throw new Error(`Không lấy được thông tin media sau 5 lần thử: ${errorMsg}`);
                    }
                }
                await this.sleep(3000);
                return false;
            ...
            const fileNameWithExt = targetFileName.endsWith(`.${base64Data.ext}`) ? targetFileName : `${targetFileName}.${base64Data.ext}`;
            const finalPath = path.join(finalDir, fileNameWithExt);

            if (base64Data.isBlob) {
                // Blob Data URL (Base64) - Dùng cho Ảnh hoặc fallback Video nhỏ
                ...
                // HTTP URL - Stream trực tiếp qua Node.js để chống OOM
                this.log(`Bắt đầu Stream file media trực tiếp cho Job ${job.id}: ${base64Data.mediaUrl.substring(0, 60)}...`);
                try {
                    ...
                    await new Promise((resolve, reject) => {
                        const req = https.get(base64Data.mediaUrl, {
                            headers: {
                                'Cookie': cookieStr,
                            ...
                            if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
                                // Handle redirect if needed (Google fifeUri often redirects)
                    ...
                    if (job.downloadRetries >= 3) {
                        throw new Error(`Lỗi stream HTTP trực tiếp media sau 3 lần thử: ${err.message}`);
                    }
                    await this.sleep(4000);
                    return false; // Thử lại sau
                }
            }

            this.master.updateJobStatus(job, 'Completed', undefined, finalPath, base64Data.mediaUrl);
            return true; // Is Done
